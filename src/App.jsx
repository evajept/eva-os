...
const Tag=({children,color})=>{const m={blue:{bg:C.blueBg,c:C.blue},green:{bg:C.greenBg,c:C.green},red:{bg:C.redBg,c:C.red},yellow:{bg:C.yellowBg,c:"#856d0a"},purple:{bg:C.purpleBg,c:C.purple},orange:{bg:C.orangeBg,c:C.orange}};const s=m[color]||{bg:C.bgS,c:C.tx};return(<span style={{display:"inline-flex",padding:"2px 8px",borderRadius:3,fontSize:15,fontFamily:F.sans,fontWeight:500,background:s.bg,color:s.c,lineHeight:"20px",whiteSpace:"nowrap"}}>{children}</span>);};
const H1=({children,style:sx})=><h1 style={{fontFamily:F.serif,fontSize:32,fontWeight:700,color:C.tx,margin:"0 0 8px",lineHeight:1.2,...sx}}>{children}</h1>;
const H2=({children,style:sx})=><h2 style={{fontFamily:F.serif,fontSize:24,fontWeight:600,color:C.tx,margin:"24px 0 8px",...sx}}>{children}</h2>;
const H3=({children,style:sx})=><h3 style={{fontFamily:F.serif,fontSize:18,fontWeight:600,color:C.tx,margin:"16px 0 6px",...sx}}>{children}</h3>;
const P=({children,style:sx})=><p style={{fontFamily:F.sans,fontSize:18,color:C.tx,lineHeight:1.65,margin:"0 0 8px",...sx}}>{children}</p>;
const Ps=({children,style:sx})=><p style={{fontFamily:F.sans,fontSize:17,color:C.txS,lineHeight:1.65,margin:"0 0 12px",...sx}}>{children}</p>;
const Pill=({children,color,bg})=><span style={{display:"inline-flex",padding:"2px 8px",borderRadius:12,fontSize:15,fontFamily:F.sans,fontWeight:500,background:bg||C.bgS,color:color||C.txS,lineHeight:"18px",whiteSpace:"nowrap"}}>{children}</span>;
const Call=({children,emoji,bg})=><div style={{padding:"12px 16px",background:bg||C.yellowBg,borderRadius:4,fontFamily:F.sans,fontSize:17,lineHeight:1.6,color:C.tx,display:"flex",gap:10,alignItems:"flex-start"}}><span style={{fontSize:18,flexShrink:0}}>{emoji}</span><div>{children}</div></div>;
...
function ABar({value}){return(<div style={{display:"flex",alignItems:"center",gap:5}}><div style={{flex:1,height:5,background:C.bgS,borderRadius:3,overflow:"hidden"}}><div style={{width:`${(value/10)*100}%`,height:"100%",background:value>=8?C.green:value>=6?C.orange:C.red,borderRadius:3}}/></div><span style={{fontSize:15,fontWeight:600,color:value>=8?C.green:value>=6?C.orange:C.red,fontFamily:F.mono,minWidth:16,textAlign:"right"}}>{value}</span></div>);}
function Collapse({title,icon,color,open:dO,children}){const[o,setO]=useState(dO||false);return(<div style={{marginBottom:6}}><div onClick={()=>setO(!o)} style={{padding:"12px 0",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:18,width:22,textAlign:"center",flexShrink:0}}>{icon}</span><span style={{fontFamily:F.sans,fontSize:18,fontWeight:600,color:C.tx,flex:1}}>{title}</span><span style={{fontSize:14,color:C.txT,transform:o?"rotate(90deg)":"rotate(0)",transition:"transform 0.15s"}}>&#9654;</span></div>{o&&<div style={{paddingTop:8}}>{children}</div>}</div>);}
const osTa=({value,onChange,placeholder,rows})=><textarea value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} rows={rows||3} style={{border:`1px solid ${C.bdr}`,borderRadius:4,padding:"10px 12px",fontSize:17,color:C.tx,background:C.bgS,outline:"none",fontFamily:F.sans,width:"100%",boxSizing:"border-box",resize:"vertical",lineHeight:1.6}}/>;
const osBtn=({children,onClick,variant,style:sx,disabled})=>{const s={primary:{background:C.green,color:"#fff"},ghost:{background:"transparent",color:C.green,border:`1px solid ${C.green}`},danger:{background:C.red,color:"#fff"}};return(<button onClick={onClick} disabled={disabled} style={{padding:"9px 18px",borderRadius:4,border:"none",cursor:disabled?"not-allowed":"pointer",fontSize:16,fontWeight:600,fontFamily:F.sans,opacity:disabled?0.6:1,...(s[variant||"primary"]||s.primary),...sx}}>{children}</button>);};
...
function GoalCard({g,isOpen,onToggle,data}){
  ...
  return(<div onClick={onToggle} style={{padding:"12px 14px",marginBottom:4,cursor:"pointer",borderRadius:4,background:isOpen?sc.bg:sc.bg,border:isOpen?`1.5px solid ${sc.c}40`:"1.5px solid transparent",transition:"all 0.15s"}}>
    <div style={{fontFamily:F.sans,fontSize:17,fontWeight:isOpen?600:500,color:C.tx}}>{g.title}</div>
    <div style={{fontSize:13,color:sc.c,fontWeight:500,marginTop:3}}>{status}</div>
  </div>);
}
function GoalDetail({g,data,setData}){
  ...
  const txtS={fontSize:16,color:C.tx,flex:1,background:"transparent",border:"none",outline:"none",fontFamily:F.sans,padding:0,minWidth:0};
  const secHdr=(label,onAdd)=>(<div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span>{label}</span><span onClick={onAdd} style={{cursor:"pointer",fontSize:17,fontWeight:400,color:C.txT}}>+</span></div>);
  const chipS=(bg,c)=>({fontSize:12,fontWeight:600,padding:"1px 6px",borderRadius:8,background:bg,color:c,cursor:"pointer",flexShrink:0,userSelect:"none"});
  ...
  const inp4={border:`1px solid ${C.bdr}`,borderRadius:3,padding:"4px 6px",fontFamily:F.sans,fontSize:15,color:C.tx,background:"transparent",outline:"none",boxSizing:"border-box"};
  ...
  return(<div style={{marginTop:8,paddingTop:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      <span style={{fontFamily:F.serif,fontSize:22,fontWeight:600}}>{g.title}</span>
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        {totalAll>0&&<span style={{fontSize:14,color:C.txT}}>{totalDone}/{totalAll} done</span>}
        <select value={d.status||"not started"} onChange={e=>set("status",e.target.value)} style={{padding:"3px 8px",borderRadius:12,fontSize:14,fontWeight:600,border:`1px solid ${C.bdr}`,background:(stCfg[d.status||"not started"]||stCfg["not started"]).bg,color:(stCfg[d.status||"not started"]||stCfg["not started"]).c,cursor:"pointer",outline:"none",fontFamily:F.sans}}>{statusOpts.map(s=><option key={s} value={s}>{s}</option>)}</select>
        <span onClick={()=>setShowCtx(!showCtx)} style={{fontSize:14,color:C.txT,cursor:"pointer",padding:"3px 8px",border:`1px solid ${C.bdr}`,borderRadius:4}}>{showCtx?"hide context":"context"}</span>
      </div>
    </div>
    {showCtx&&<div style={{background:C.bgS,borderRadius:4,padding:"12px 16px",marginBottom:16}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:20}}>
        <div>
          <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:4,textTransform:"uppercase",letterSpacing:"0.04em"}}>Vision</div>
          <textarea value={d.vision||""} onChange={e=>set("vision",e.target.value)} rows={3} placeholder="How does accomplishing this look like?" style={{...inp4,width:"100%",fontSize:15,resize:"vertical",lineHeight:1.5}}/>
          <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:4,marginTop:10,textTransform:"uppercase",letterSpacing:"0.04em"}}>Why</div>
          <textarea value={d.why||""} onChange={e=>set("why",e.target.value)} rows={2} placeholder="Why does this matter?" style={{...inp4,width:"100%",fontSize:15,resize:"vertical",lineHeight:1.5}}/>
        </div>
        <div>
          <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em",display:"flex",justifyContent:"space-between"}}><span>Assess</span><span onClick={addAssess} style={{cursor:"pointer",fontSize:15,fontWeight:400}}>+</span></div>
          {(d.assess||[]).map((a,i)=>(<div key={i} style={{padding:"4px 0",borderBottom:`1px solid ${C.bdr}`,display:"flex",alignItems:"center",gap:8}}>
            <input value={a.metric} onChange={e=>{const u=[...(d.assess||[])];u[i]={...u[i],metric:e.target.value};set("assess",u);}} style={{...txtS,fontSize:15,fontWeight:500}} placeholder="Metric"/>
            <input value={a.target} onChange={e=>{const u=[...(d.assess||[])];u[i]={...u[i],target:e.target.value};set("assess",u);}} style={{...txtS,fontSize:15,color:C.txS,textAlign:"right",flex:"0 0 auto",width:100}} placeholder="Target"/>
          </div>))}
        </div>
        <div>
          <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em",display:"flex",justifyContent:"space-between"}}><span>Practices</span><span onClick={addPractice} style={{cursor:"pointer",fontSize:15,fontWeight:400}}>+</span></div>
          {practices.map((p,i)=>(<div key={p.id} style={{padding:"4px 0",borderBottom:`1px solid ${C.bdr}`,display:"flex",alignItems:"center",gap:8}}>
            <input value={p.text} onChange={e=>updPractice(p.id,"text",e.target.value)} style={{...txtS,fontSize:15}} placeholder="Practice..."/>
            <span onClick={()=>{const freqs=["daily","5/w","3/w","1/w","1/m"];const idx=freqs.indexOf(p.freq);updPractice(p.id,"freq",freqs[(idx+1)%freqs.length]);}} style={{fontSize:14,color:C.txT,cursor:"pointer",flexShrink:0}}>{p.freq||"daily"}</span>
            <span onClick={()=>rmPractice(p.id)} style={{cursor:"pointer",fontSize:14,color:C.txT,flexShrink:0}}>x</span>
          </div>))}
        </div>
      </div>
    </div>}
      ...
      <div>
        ...
        {doneActions.length>0&&<div style={{background:C.bgS,borderRadius:4,padding:"6px 10px",marginTop:4}}>
          <div style={{display:"flex",justifyContent:"flex-end",alignItems:"center",cursor:"pointer",padding:"2px 0"}} onClick={()=>setShowDoneA(!showDoneA)}>
            <span style={{fontSize:11,color:C.txT,transform:showDoneA?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.15s",lineHeight:1}}>{"\u25BC"}</span>
          </div>
          {showDoneA&&doneActions.map((a,i)=>(<div key={a.id} style={{padding:"4px 0",display:"flex",alignItems:"center",gap:6,borderBottom:i<doneActions.length-1?`1px solid ${C.bdr}`:"none"}}>
            ...
            <span style={{fontSize:15,color:C.txT,textDecoration:"line-through",flex:1}}>{a.text}</span>
            {a.date&&<span style={{fontSize:13,color:C.txT,flexShrink:0}}>{a.date}</span>}
          </div>))}
        </div>}
      </div>
      <div>
        ...
        {todoLearning.map(l=>(<div key={l.id} style={lineS}>
          ...
          <span onClick={()=>rmLearning(l.id)} style={{cursor:"pointer",fontSize:14,color:C.txT,flexShrink:0}}>x</span>
        </div>))}
        {doneLearning.length>0&&<div style={{background:C.bgS,borderRadius:4,padding:"6px 10px",marginTop:4}}>
          <div style={{display:"flex",justifyContent:"flex-end",alignItems:"center",cursor:"pointer",padding:"2px 0"}} onClick={()=>setShowDoneL(!showDoneL)}>
            <span style={{fontSize:11,color:C.txT,transform:showDoneL?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.15s",lineHeight:1}}>{"\u25BC"}</span>
          </div>
          {showDoneL&&doneLearning.map((l,i)=>{const st=l.status||"done";const sc=learnStatusCfg[st]||learnStatusCfg.done;return(<div key={l.id} style={{padding:"4px 0",display:"flex",alignItems:"center",gap:6,borderBottom:i<doneLearning.length-1?`1px solid ${C.bdr}`:"none"}}>
            <span style={{fontSize:15,color:C.txT,flex:1}}>{l.text}</span>
  ...
  if(view==="plans"&&planGoal){
    ...
    <div style={{display:"flex",alignItems:"center",gap:0,padding:"6px 0",marginBottom:8,overflowX:"auto",flexWrap:"nowrap",borderBottom:`1px solid ${C.bdr}`}}>
      {[{goals:cores,color:C.purple},{goals:projects,color:C.gold},{goals:experiences,color:C.ocean}].map((grp,gi)=>(<div key={gi} style={{display:"flex",alignItems:"center",gap:0,flexShrink:0}}>
        {gi>0&&<span style={{fontSize:14,fontWeight:700,color:C.tx,padding:"0 3px",flexShrink:0}}>/</span>}
        {grp.goals.map((g,i)=>(<span key={g.n} style={{display:"flex",alignItems:"center",flexShrink:0}}>
          {i>0&&<span style={{fontSize:14,color:C.txT,padding:"0 1px",flexShrink:0}}>/</span>}
          <span onClick={()=>setActiveGoal(g.n)} style={{fontSize:15,fontWeight:planGoal.n===g.n?600:400,color:planGoal.n===g.n?C.tx:C.txT,cursor:"pointer",padding:"3px 6px",whiteSpace:"nowrap",borderBottom:planGoal.n===g.n?`2px solid ${grp.color}`:"2px solid transparent",marginBottom:-1}}>{g.short}</span>
        </span>))}
      </div>))}</div>
...
function HabitsTab(){
  ...
  const cellS={width:17,height:17,borderRadius:3,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:600,cursor:"pointer",userSelect:"none",fontFamily:F.mono,flexShrink:0};
  const chkS={...cellS,width:15,height:15};

  return(<div><H1>Habits</H1>
    <div style={{display:"flex",gap:14,alignItems:"center",marginBottom:16}}>
      <input type="month" value={viewDate} onChange={e=>{setViewDate(e.target.value);setLoaded(false);}} style={{border:`1px solid ${C.bdr}`,borderRadius:4,padding:"8px 12px",fontSize:17,color:C.tx,background:C.bgS,outline:"none",fontFamily:F.sans}}/>
      <div style={{fontSize:24,fontWeight:700,color:overallPct>=70?C.green:overallPct>=40?C.orange:C.red}}>{overallPct}%</div>
    </div>
    <div style={{border:`1px solid ${C.bdr}`,borderRadius:4,overflow:"hidden"}}>
      <div style={{overflowX:"auto"}}>
        <table style={{borderCollapse:"collapse",width:"100%",minWidth:700}}>
          <thead><tr>
            <th style={{textAlign:"left",fontSize:14,color:C.txS,padding:"8px 8px",minWidth:160}}>Habit</th>
            {Array.from({length:days},(_,i)=><th key={i+1} style={{fontSize:12,color:C.txT,padding:"3px 1px",minWidth:17,textAlign:"center"}}>{i+1}</th>)}
            <th style={{fontSize:14,color:C.txS,padding:"4px 8px",textAlign:"right",minWidth:36}}>%</th>
          </tr></thead>
          <tbody>
            {HABIT_SECTIONS.map(section=>(<React.Fragment key={section.key}>
              <tr><td colSpan={days+2} style={{padding:"6px 8px",fontSize:14,fontWeight:600,color:C.txT,background:C.bgS,borderBottom:`1px solid ${C.bdr}`,borderTop:`1px solid ${C.bdr}`,textTransform:"uppercase",letterSpacing:"0.04em"}}>{section.label}</td></tr>
              {section.items.map(item=>{const stat=getStat(item);return(
                <tr key={item.id} style={{borderBottom:`1px solid ${C.bdr}`}}>
                  <td style={{padding:"5px 8px",fontSize:15,color:C.tx,whiteSpace:"nowrap"}}>{item.label}</td>
                  ...
                  <td style={{padding:"4px 8px",textAlign:"right",fontSize:15,fontWeight:600,color:stat.color}}>{stat.val}</td>
                </tr>);})}
            </React.Fragment>))}
          </tbody>
        </table>
    ...
    <div style={{display:"flex",gap:16,flexWrap:"wrap",marginTop:10,padding:"8px 0",fontSize:13,color:C.txT}}>
...
const BLOOD_GROUPS = [
  { key: "glucose", label: "Glucose", markers: [
    { id: "glucose", label: "Fasting glucose", norm: "70-99", good: (v) => v <= 99, warn: (v) => v <= 110 },
    { id: "hba1c", label: "HbA1c", norm: "<5.7", good: (v) => v <= 5.7, warn: (v) => v <= 6.0 },
  ]},
  { key: "lipid", label: "Lipid profile", markers: [
    { id: "chol", label: "Total cholesterol", norm: "<200", good: (v) => v <= 200, warn: (v) => v <= 240 },
    { id: "hdl", label: "HDL", norm: ">50", good: (v) => v >= 50, warn: (v) => v >= 40 },
    { id: "ldl_calc", label: "LDL (calculated)", norm: "<100", good: (v) => v <= 100, warn: (v) => v <= 130 },
    { id: "ldl_direct", label: "LDL (direct)", norm: "<100", good: (v) => v <= 100, warn: (v) => v <= 130 },
    { id: "tg", label: "Triglycerides", norm: "<150", good: (v) => v <= 150, warn: (v) => v <= 200 },
    { id: "tchdl", label: "TC/HDL ratio", norm: "<5.0", good: (v) => v <= 5.0, warn: (v) => v <= 6.0 },
  ]},
  { key: "thyroid", label: "Thyroid", markers: [
    { id: "tsh", label: "TSH", norm: "0.4-4.0", good: (v) => v >= 0.4 && v <= 4.0, warn: (v) => v <= 5.0 },
    { id: "ft4", label: "Free T4", norm: "0.89-1.76", good: (v) => v >= 0.89 && v <= 1.76, warn: () => true },
    { id: "ft3", label: "Free T3", norm: "2.3-4.2", good: (v) => v >= 2.3 && v <= 4.2, warn: () => true },
    { id: "antitpo", label: "Anti-TPO", norm: "<5.61", good: (v) => v <= 5.61, warn: (v) => v <= 15 },
    { id: "antiatg", label: "Anti-ATG", norm: "<4.11", good: (v) => v <= 4.11, warn: (v) => v <= 10 },
  ]},
  { key: "liver", label: "Liver", markers: [
    { id: "alt", label: "ALT (SGPT)", norm: "<33", good: (v) => v <= 33, warn: (v) => v <= 56 },
    { id: "ast", label: "AST (SGOT)", norm: "<32", good: (v) => v <= 32, warn: (v) => v <= 40 },
    { id: "ggt", label: "GGT", norm: "<38", good: (v) => v <= 38, warn: (v) => v <= 50 },
  ]},
  { key: "kidney", label: "Kidney", markers: [
    { id: "bun", label: "BUN", norm: "9-23", good: (v) => v >= 9 && v <= 23, warn: (v) => v >= 7 && v <= 25 },
    { id: "creatinine", label: "Creatinine", norm: "0.55-1.02", good: (v) => v >= 0.55 && v <= 1.02, warn: (v) => v <= 1.2 },
    { id: "egfr", label: "eGFR", norm: ">90", good: (v) => v >= 90, warn: (v) => v >= 60 },
    { id: "calcium", label: "Calcium", norm: "8.7-10.4", good: (v) => v >= 8.7 && v <= 10.4, warn: () => true },
    { id: "uricacid", label: "Uric acid", norm: "3.1-7.8", good: (v) => v >= 3.1 && v <= 7.8, warn: () => true },
  ]},
  { key: "blood", label: "Blood count", markers: [
    { id: "rbc", label: "RBC", norm: "3.9-5.6", good: (v) => v >= 3.9 && v <= 5.6, warn: () => true },
    { id: "hgb", label: "Haemoglobin", norm: "11.5-16.5", good: (v) => v >= 11.5 && v <= 16.5, warn: (v) => v >= 10 },
    { id: "hct", label: "Haematocrit", norm: "35-47", good: (v) => v >= 35 && v <= 47, warn: () => true },
    { id: "mcv", label: "MCV", norm: "80-99", good: (v) => v >= 80 && v <= 99, warn: (v) => v >= 75 && v <= 105 },
    { id: "wbc", label: "WBC", norm: "4.0-10.0", good: (v) => v >= 4 && v <= 10, warn: (v) => v >= 3 && v <= 12 },
    { id: "plt", label: "Platelet count", norm: "140-450", good: (v) => v >= 140 && v <= 450, warn: () => true },
    { id: "rdw", label: "RDW", norm: "11.5-14.5", good: (v) => v >= 11.5 && v <= 14.5, warn: () => true },
  ]},
  { key: "urine", label: "Urine", markers: [
    { id: "urine_ph", label: "pH", norm: "5.0-8.0", good: (v) => v >= 5 && v <= 8, warn: () => true },
    { id: "urine_protein", label: "Protein", norm: "Negative", good: () => true, warn: () => true, isText: true },
    { id: "urine_glucose", label: "Glucose (urine)", norm: "Negative", good: () => true, warn: () => true, isText: true },
  ]},
  { key: "body", label: "Body", markers: [
    { id: "height", label: "Height", norm: "-", good: () => true, warn: () => true },
    { id: "weight", label: "Weight", norm: "Target: 50", good: (v) => v <= 52, warn: (v) => v <= 55 },
    { id: "bmi", label: "BMI", norm: "18.5-24.9", good: (v) => v >= 18.5 && v <= 24.9, warn: (v) => v <= 27 },
    { id: "bodyfat", label: "Body fat", norm: "21-33", good: (v) => v >= 21 && v <= 33, warn: () => true },
    { id: "waist", label: "Waist", norm: "<80", good: (v) => v <= 80, warn: (v) => v <= 88 },
    { id: "hips", label: "Hips", norm: "-", good: () => true, warn: () => true },
    { id: "bust", label: "Bust", norm: "-", good: () => true, warn: () => true },
    { id: "arms", label: "Arms", norm: "-", good: () => true, warn: () => true },
    { id: "thighs", label: "Thighs", norm: "-", good: () => true, warn: () => true },
    { id: "neck", label: "Neck", norm: "-", good: () => true, warn: () => true },
    { id: "shoulders", label: "Shoulders", norm: "-", good: () => true, warn: () => true },
  ]},
];
...
function MetricsTab() {
  ...
  const grpS={flex:"0 0 100px",fontSize:15,fontWeight:600,padding:"4px 10px",color:C.tx,alignSelf:"stretch",display:"flex",alignItems:"center"};
  const grpE={flex:"0 0 100px",padding:"4px 10px"};
  const nameS={flex:"0 0 160px",paddingLeft:8,color:C.tx,fontSize:15};
  const normS={flex:"0 0 90px",fontSize:14,textAlign:"left",color:C.txT,paddingLeft:4};
  const valS={flex:"0 0 54px",textAlign:"center",fontSize:15,fontWeight:500};
  const dotS={width:5,height:5,borderRadius:3,flexShrink:0,margin:"0 4px 0 2px"};
  const rowS={display:"flex",alignItems:"center",padding:"3px 0",fontSize:15};
  const futureS={...valS,color:C.txT,fontWeight:400,fontSize:14};
  const hdrValS={...valS,fontSize:13,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.04em",color:C.txT};
  ...
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:10,marginBottom:20}}>
      {[
        ...
        <div style={{fontSize:13,fontWeight:600,color:C.txT,textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:3}}>{g.l}</div>
        <div style={{fontSize:22,fontWeight:500,color:g.c}}>{g.v}</div>
        <div style={{fontSize:13,color:C.txT,marginTop:2}}>{g.sub}</div>
      </div>))}
    </div>

    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
      <span style={{fontSize:17,fontWeight:600,color:C.tx}}>Markers</span>
      <span onClick={openModal} style={{padding:"5px 12px",fontSize:14,fontWeight:600,background:C.green,color:"#fff",borderRadius:4,cursor:"pointer"}}>+ Add blood work</span>
    </div>

    <div style={{display:"flex",alignItems:"center",padding:"6px 0",borderBottom:`0.5px solid ${C.bdr}`}}>
      <span style={grpE}></span>
      <span style={{...nameS,fontSize:13,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.04em",color:C.txT}}>Marker</span>
      <span style={{...normS,fontSize:13,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.04em"}}>Normal</span>
    ...
    {entries.length>0&&<div style={{marginTop:8,display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
      {entries.map((entry,i)=>(<div key={entry.id} style={{display:"flex",alignItems:"center",gap:6,fontSize:14,color:C.txT}}>
        <span style={{fontSize:14,color:i===0?C.tx:C.txT,fontWeight:i===0?500:400}}>{fmtDate(entry.date)}</span>
        {i===0&&<span style={{fontSize:12,fontWeight:600,padding:"1px 6px",borderRadius:8,background:C.greenBg,color:C.green}}>latest</span>}
        <span onClick={()=>deleteEntry(entry.id)} style={{cursor:"pointer",fontSize:13,color:C.txT}}>x</span>
      </div>))}
    </div>}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,alignItems:"start"}}>
      <div>
        <div style={{background:C.bgS,padding:"10px 16px",borderBottom:`1px solid ${C.bdr}`}}><span style={{fontSize:17,fontWeight:600,color:C.tx}}>Health conditions</span></div>
        {CONDITIONS.map(c=>{const status=(data.conditions||{})[c.id]||"monitoring";const sc=COND_COLORS[status]||COND_COLORS.monitoring;return(
          <div key={c.id} style={lineS}>
            <span style={{fontSize:16,color:C.tx,flex:1,paddingLeft:16}}>{c.label}</span>
            <span onClick={()=>cycleCondition(c.id)} style={{fontSize:13,fontWeight:500,padding:"2px 10px",borderRadius:10,background:sc.bg,color:sc.c,cursor:"pointer",userSelect:"none"}}>{status}</span>
            <span style={{fontSize:14,color:C.txT,width:80,textAlign:"right"}}>{c.target}</span>
          </div>);})}
      </div>
      <div>
        <div style={{background:C.bgS,padding:"10px 16px",borderBottom:`1px solid ${C.bdr}`}}><span style={{fontSize:17,fontWeight:600,color:C.tx}}>Supplement protocol</span></div>
        {SUPPLEMENTS.map((s,i)=>(<div key={i} style={lineS}>
          <span style={{fontSize:16,color:C.tx,flex:1,paddingLeft:16}}>{s.name}</span>
          <span style={{fontSize:14,color:C.txS,width:60}}>{s.dose}</span>
          <span style={{fontSize:14,color:C.txT,width:60}}>{s.time}</span>
        </div>))}
      </div>
    </div>
    ...
    {showModal&&<div style={modalOverlay} onClick={closeModal}><div style={modalBox} onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <span style={{fontSize:20,fontWeight:500,fontFamily:F.serif,color:C.tx}}>Add blood work</span>
        <span onClick={closeModal} style={{fontSize:16,color:C.txT,cursor:"pointer"}}>x</span>
      </div>
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        {[{k:"upload",l:"Upload"},{k:"type",l:"Type values"}].map(t=>(
          <div key={t.k} onClick={()=>{setModalTab(t.k);setExtractedValues(null);setExtractError(null);}} style={{flex:1,padding:"10px 12px",borderRadius:4,border:modalTab===t.k?`2px solid ${C.blue}`:`1px solid ${C.bdr}`,textAlign:"center",cursor:"pointer",background:modalTab===t.k?C.blueBg:"transparent"}}>
            <div style={{fontSize:15,fontWeight:500,color:modalTab===t.k?C.blue:C.tx}}>{t.l}</div>
          </div>
        ))}
      </div>
      ...
      {modalTab==="upload"&&<div>
        ...
        <div ref={dropRef} onDrop={handleDrop} onDragOver={handleDragOver} onClick={()=>fileInputRef.current?.click()} style={{border:`2px dashed ${C.bdrH}`,borderRadius:4,padding:"24px 20px",textAlign:"center",marginBottom:12,cursor:"pointer"}}>
          <div style={{fontSize:16,color:C.txS,marginBottom:4}}>Drop, paste, or click to browse</div>
          <div style={{fontSize:14,color:C.txT}}>JPG, PNG, PDF - max 10MB per file</div>
        </div>
        {modalFiles.length>0&&<div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
          {modalFiles.map((f,i)=>(
            <div key={i} style={{flex:"0 0 80px",height:50,borderRadius:4,background:C.bgS,border:`1px solid ${C.bdr}`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
              <div style={{fontSize:13,color:C.txS,textAlign:"center",padding:"0 4px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:72}}>{f.name.length>12?f.name.slice(0,10)+"..":f.name}</div>
              <span onClick={()=>removeFile(i)} style={{position:"absolute",top:-6,right:-6,width:16,height:16,borderRadius:8,background:C.redBg,color:C.red,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>x</span>
            </div>
          ))}
          <div onClick={()=>fileInputRef.current?.click()} style={{flex:"0 0 50px",height:50,borderRadius:4,border:`1px dashed ${C.bdr}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
            <span style={{fontSize:20,color:C.txT}}>+</span>
          </div>
        </div>}
        {extracting&&<div style={{padding:16,textAlign:"center",fontSize:15,color:C.txS}}>Extracting values...</div>}
        {extractError&&<div style={{padding:10,background:C.redBg,borderRadius:4,fontSize:14,color:C.red,marginBottom:12}}>{extractError}</div>}
      </div>}
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
        <span style={{fontSize:14,color:C.txS}}>Date</span>
        <input type="date" value={modalDate} onChange={e=>setModalDate(e.target.value)} style={{flex:1,border:`1px solid ${C.bdr}`,borderRadius:3,padding:"6px 8px",fontSize:14,fontFamily:F.sans,color:C.tx,background:"transparent",outline:"none"}}/>
      </div>

      {extractedValues&&<div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
          <span style={{fontSize:13,fontWeight:500,color:C.txT,textTransform:"uppercase",letterSpacing:"0.04em"}}>Extracted values</span>
          <span style={{fontSize:13,color:C.green}}>{Object.keys(extractedValues).filter(k=>extractedValues[k]!==""&&extractedValues[k]!==undefined).length} markers found</span>
        </div>
        <div style={{background:C.bgS,borderRadius:4,padding:"8px 12px",marginBottom:8,maxHeight:200,overflowY:"auto"}}>
          {mergedGroups.map(grp=>{
            ...
            return(<div key={grp.key}>
              {grpVals.map((m,mi)=>{const val=extractedValues[m.id];const color=getMarkerColor(m,val);return(<div key={m.id} style={{display:"flex",alignItems:"center",padding:"2px 0",fontSize:14}}>
                {mi===0?<span style={{flex:"0 0 80px",fontSize:13,fontWeight:500,color:C.txS}}>{grp.label}</span>:<span style={{flex:"0 0 80px"}}></span>}
        ...
        {newMarkers.length>0&&<div style={{background:C.yellowBg,borderRadius:4,padding:"8px 12px",marginBottom:8}}>
          <div style={{fontSize:14,fontWeight:500,color:"#856d0a",marginBottom:4}}>{newMarkers.length} new marker{newMarkers.length>1?"s":""} detected</div>
          {newMarkers.map((nm,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"2px 0",fontSize:14}}>
              ...
              <span style={{fontSize:13,color:C.txT}}>({nm.group})</span>
            </div>
          ))}
          <div style={{fontSize:13,color:"#856d0a",marginTop:4}}>Check to add these markers to your tracker</div>
        </div>}
      </div>}

      <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:8}}>
        <span onClick={closeModal} style={{padding:"7px 16px",fontSize:15,color:C.txS,cursor:"pointer",border:`1px solid ${C.bdr}`,borderRadius:4}}>Cancel</span>
        <span onClick={saveEntry} style={{padding:"7px 16px",fontSize:15,fontWeight:600,color:valCount>0?"#fff":C.txT,background:valCount>0?C.green:C.bgS,borderRadius:4,cursor:valCount>0?"pointer":"default"}}>{valCount>0?"Save "+valCount+" markers":"Save"}</span>
      </div>
    </div></div>}
  </div>);
}
...
function ProtocolTab() {
  ...
  const labelS = { fontSize: 12, fontWeight: 600, color: C.txT, textTransform: "uppercase", letterSpacing: "0.04em", minWidth: 80, paddingTop: 2, flexShrink: 0 };
  ...
  return (<div>
    ...
    <div style={{ background: C.bgS, padding: "10px 16px", borderBottom: `1px solid ${C.bdr}` }}><span style={{ fontSize: 15, fontWeight: 600, color: C.tx }}>Daily stack</span></div>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
      {PROTOCOL_DATA.map((s, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", borderBottom: `1px solid ${C.bdr}` }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 500, color: C.tx, flex: 1 }}>{s.name}</span>
        <span style={{ fontSize: 12, color: C.txS }}>{s.dose}</span>
        <span style={{ fontSize: 12, color: C.txT }}>{s.timing.split(",")[0]}</span>
      </div>))}
    </div>
    <div style={{ height: 20 }} />
    {PROTOCOL_DATA.map((s, i) => (<div key={i} style={{ marginBottom: 16 }}>
      <div style={{ background: C.bgS, padding: "12px 16px", borderBottom: `1px solid ${C.bdr}`, display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 600, color: C.tx }}>{s.name}</span>
        <span style={{ fontSize: 13, color: C.txS, marginLeft: "auto" }}>{s.dose} - {s.timing}</span>
      </div>
      <div style={{ padding: "10px 16px" }}>
        <div style={lineS}><span style={labelS}>Target</span><span style={{ fontSize: 14, color: s.color, fontWeight: 500 }}>{s.target}</span></div>
        <div style={lineS}><span style={labelS}>What</span><span style={{ fontSize: 14, color: C.tx, lineHeight: 1.6 }}>{s.what}</span></div>
        <div style={lineS}><span style={labelS}>Why you</span><span style={{ fontSize: 14, color: C.tx, lineHeight: 1.6 }}>{s.why}</span></div>
        <div style={lineS}><span style={labelS}>Watch for</span><span style={{ fontSize: 14, color: C.tx, lineHeight: 1.6 }}>{s.watch}</span></div>
        <div style={lineS}><span style={labelS}>Interactions</span><span style={{ fontSize: 14, color: C.tx, lineHeight: 1.6 }}>{s.interactions}</span></div>
        <div style={{ ...lineS, borderBottom: "none" }}><span style={labelS}>Adjust</span><span style={{ fontSize: 14, color: C.tx, lineHeight: 1.6 }}>{s.adjust}</span></div>
      </div>
    </div>))}
    <div style={{ background: C.bgS, padding: "12px 16px", borderRadius: 4, marginTop: 8 }}>
      <div style={{ fontSize: 14, fontWeight: 600, color: C.tx, marginBottom: 6 }}>General guidelines</div>
  ...
  return (<div>
    ...
    <div style={{ display: "flex", gap: 0, borderBottom: `1px solid ${C.bdr}`, marginBottom: 16 }}>
      {[{ k: "standard", l: "Standard" }, { k: "stream", l: "Streaming" }, { k: "tools", l: "Tool use" }, { k: "ab", l: "A/B test" }].map(t => (
        <button key={t.k} onClick={() => setMode(t.k)} style={{ padding: "7px 14px", border: "none", background: "none", fontFamily: F.sans, fontSize: 14, fontWeight: mode === t.k ? 600 : 400, color: mode === t.k ? C.tx : C.txT, cursor: "pointer", borderBottom: mode === t.k ? `2px solid ${C.green}` : "2px solid transparent", marginBottom: -1 }}>{t.l}</button>
      ))}
    </div>

    <div style={{ fontSize: 13, color: C.txT, marginBottom: 12, fontFamily: F.sans }}>
    ...
    {alerts.length > 0 && (<div style={{ marginBottom: 16 }}>
      {alerts.map((a, i) => (<div key={i} style={{ padding: "8px 12px", marginBottom: 4, borderRadius: 4, fontSize: 14, background: a.type === "danger" ? C.redBg : C.yellowBg, color: a.type === "danger" ? C.red : "#856d0a", display: "flex", gap: 8, alignItems: "center" }}><span style={{ fontSize: 15, flexShrink: 0 }}>{a.type === "danger" ? "\u26A0" : "\u25CF"}</span>{a.text}</div>))}
    </div>)}

    {error && <div style={{ padding: "12px 16px", background: C.redBg, borderRadius: 4, marginBottom: 16, fontSize: 14, color: C.red }}>{error}</div>}

    {isStreaming && streamText && (<pre style={{ padding: "14px 16px", background: C.bgS, borderRadius: 4, fontSize: 13, color: C.txS, fontFamily: F.mono, whiteSpace: "pre-wrap", lineHeight: 1.6, marginBottom: 16, overflow: "auto", maxHeight: 300 }}>{streamText}<span style={{ animation: "blink 1s infinite" }}>|</span></pre>)}

    {toolLog.length > 0 && (<div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: C.purple, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 6, fontFamily: F.sans }}>Tool calls</div>
      {toolLog.map((t, i) => (<div key={i} style={{ padding: "6px 10px", marginBottom: 3, borderRadius: 4, fontSize: 13, background: C.purpleBg, color: C.purple, fontFamily: F.mono }}>{t.tool}({JSON.stringify(t.input)})</div>))}
    </div>)}
    ...
    {abResult && (<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
      {[{ label: "A: Health Officer", data: abResult.a, color: C.green }, { label: "B: Clinical Analyst", data: abResult.b, color: C.purple }].map(({ label, data: d, color }) => (
        <div key={label} style={{ border: `1px solid ${C.bdr}`, borderRadius: 6, padding: "12px 14px" }}>
          <div style={{ fontSize: 12, fontWeight: 600, color, marginBottom: 8, fontFamily: F.sans }}>{label}</div>
          <div style={{ fontSize: 22, fontWeight: 500, color: scoreColor(d.overall_score), marginBottom: 6 }}>{d.overall_score}/10</div>
          <div style={{ fontSize: 13, color: C.txS, marginBottom: 4, lineHeight: 1.5 }}>{d.top_pattern}</div>
          <div style={{ fontSize: 13, color: C.red, marginBottom: 4, lineHeight: 1.5 }}>{d.concern}</div>
          <div style={{ fontSize: 13, color: C.green, fontWeight: 500, lineHeight: 1.5 }}>{d.recommendation}</div>
          {d.reasoning && (<div style={{ fontSize: 12, color: C.txT, marginTop: 8, lineHeight: 1.5, fontStyle: "italic" }}>{d.reasoning.slice(0, 200)}...</div>)}
        </div>
      ))}
    </div>)}
    ...
    {analysis && (<div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
        {[{ l: "Overall", v: analysis.overall_score + "/10", c: scoreColor(analysis.overall_score) }, { l: "Sleep avg", v: (analysis.sleep_avg || "-") + "h", c: analysis.sleep_avg >= 7 ? C.green : C.orange, extra: analysis.sleep_trend }, { l: "Water avg", v: (analysis.water_avg || "-") + "L", c: (analysis.water_avg || 0) >= 1.5 ? C.green : C.orange }, { l: "Supplements", v: analysis.supplement_compliance || "-", c: C.tx }].map((g, i) => (
          <div key={i} style={{ background: C.bgS, borderRadius: 4, padding: "10px 12px" }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}><span style={{ fontSize: 22, fontWeight: 500, color: g.c }}>{g.v}</span>{g.extra && <span style={{ fontSize: 15, color: trendColor(g.extra) }}>{trendArrow(g.extra)}</span>}</div>
          </div>
        ))}
      </div>

      {analysis.reasoning && (<div style={{ marginBottom: 16 }}>
        <div onClick={() => setShowReasoning(!showReasoning)} style={{ fontSize: 12, color: C.purple, cursor: "pointer", fontFamily: F.sans, fontWeight: 600, padding: "6px 0" }}>{showReasoning ? "Hide" : "Show"} Health Officer's reasoning</div>
        {showReasoning && (<div style={{ padding: "12px 16px", background: C.purpleBg, borderRadius: 4, fontSize: 14, color: C.tx, lineHeight: 1.6, fontStyle: "italic" }}>{analysis.reasoning}</div>)}
      </div>)}

      {analysis.weekly_summary && (<div style={{ padding: "12px 16px", background: C.blueBg, borderRadius: 4, marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: C.blue, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 4 }}>Weekly summary</div>
        <div style={{ fontSize: 15, color: C.tx, lineHeight: 1.6 }}>{analysis.weekly_summary}</div>
        {analysis.trend_insight && <div style={{ fontSize: 13, color: C.txS, marginTop: 6, fontStyle: "italic" }}>{analysis.trend_insight}</div>}
      </div>)}
      ...
      {[{ label: "Top pattern", value: analysis.top_pattern, color: C.txT }, { label: "Concern", value: analysis.concern, color: C.red }, { label: "Action", value: analysis.recommendation, color: C.green }].map((row, i) => (
        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "10px 0", borderBottom: `1px solid ${C.bdr}` }}>
          <span style={{ fontSize: 11, fontWeight: 600, color: row.color, textTransform: "uppercase", letterSpacing: "0.04em", minWidth: 100, paddingTop: 2, flexShrink: 0, fontFamily: F.sans }}>{row.label}</span>
          <span style={{ fontSize: 15, color: row.label === "Concern" && row.value === "None" ? C.green : C.tx, lineHeight: 1.5, fontWeight: row.label === "Action" ? 500 : 400 }}>{row.value}</span>
        </div>
      ))}

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 12 }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: 600, color: C.green, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 6, fontFamily: F.sans }}>Wins</div>
          {(analysis.wins || []).map((w, i) => <div key={i} style={{ fontSize: 14, color: C.tx, padding: "4px 0", borderBottom: `1px solid ${C.bdr}` }}>{w}</div>)}
        </div>
        <div>
          <div style={{ fontSize: 11, fontWeight: 600, color: C.red, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 6, fontFamily: F.sans }}>Flags</div>
          {(analysis.flags || []).length > 0 ? (analysis.flags || []).map((f, i) => <div key={i} style={{ fontSize: 14, color: C.red, padding: "4px 0", borderBottom: `1px solid ${C.bdr}` }}>{f}</div>) : <div style={{ fontSize: 14, color: C.txT }}>None</div>}
        </div>
      </div>

      <div style={{ marginTop: 16 }}>
        <span onClick={() => setShowRaw(!showRaw)} style={{ fontSize: 12, color: C.txT, cursor: "pointer" }}>{showRaw ? "Hide" : "Show"} raw API response</span>
        {showRaw && rawJSON && <pre style={{ marginTop: 8, padding: "12px 14px", background: C.bgS, borderRadius: 4, fontSize: 12, color: C.txS, fontFamily: F.mono, whiteSpace: "pre-wrap", lineHeight: 1.6, overflow: "auto" }}>{rawJSON}</pre>}
      </div>
    </div>)}

    <div style={{ marginTop: 24, borderTop: `1px solid ${C.bdr}`, paddingTop: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: C.txT, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 8, fontFamily: F.sans }}>Chat with Health Officer</div>
      <div style={{ maxHeight: 300, overflowY: "auto", marginBottom: 10 }}>
        {chatMessages.map((m, i) => (<div key={i} style={{ padding: "8px 12px", marginBottom: 4, borderRadius: 6, fontSize: 14, lineHeight: 1.6, background: m.role === "user" ? C.bgS : C.greenBg, color: C.tx, fontFamily: F.sans }}><span style={{ fontSize: 11, fontWeight: 600, color: m.role === "user" ? C.txT : C.green, marginRight: 6 }}>{m.role === "user" ? "You" : "Health Officer"}</span>{m.text}</div>))}
        {chatLoading && <div style={{ fontSize: 13, color: C.txT, padding: "8px 12px" }}>Thinking...</div>}
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && sendChat()} placeholder="Ask a follow-up question..." style={{ flex: 1, border: `1px solid ${C.bdr}`, borderRadius: 4, padding: "8px 12px", fontSize: 14, color: C.tx, background: C.bgS, outline: "none", fontFamily: F.sans }} />
    ...
    {history.length > 0 && (<div style={{ marginTop: 24 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: C.txT, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 8, fontFamily: F.sans }}>Previous analyses</div>
      {history.map((h, i) => {
        ...
        return (<div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: `1px solid ${C.bdr}` }}>
          <span onClick={() => { setAnalysis(h.analysis); setAlerts(h.alerts || []); setRawJSON(h.rawJSON || null); }} style={{ fontSize: 14, color: C.blue, cursor: "pointer", flex: 1 }}>{label}</span>
          {h.source && <span style={{ fontSize: 10, color: C.txT, padding: "1px 6px", borderRadius: 6, background: C.bgS }}>{h.source}</span>}
          <span style={{ fontSize: 15, fontWeight: 600, color: scoreColor(h.analysis.overall_score) }}>{h.analysis.overall_score}/10</span>
          <span onClick={() => rateAnalysis(i, "up")} style={{ fontSize: 15, cursor: "pointer", opacity: h.rating === "up" ? 1 : 0.3 }}>&#x1F44D;</span>
          <span onClick={() => rateAnalysis(i, "down")} style={{ fontSize: 15, cursor: "pointer", opacity: h.rating === "down" ? 1 : 0.3 }}>&#x1F44E;</span>
          <span onClick={() => deleteHistoryEntry(i)} style={{ fontSize: 12, color: C.txT, cursor: "pointer", padding: "2px 6px" }}>x</span>
        </div>);
      })}
    </div>)}

    <div style={{ marginTop: 24, padding: "12px 16px", background: C.bgS, borderRadius: 4 }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: C.txT, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 6, fontFamily: F.sans }}>CCA skills practiced in this tab</div>
      <div style={{ fontSize: 13, color: C.txS, lineHeight: 1.8, fontFamily: F.sans }}>
...
function BodyPage() {
  ...
  return (<div>
    <H1 style={{ fontSize: 30, margin: "0 0 4px" }}>Body</H1>
    <div style={{ display: "flex", gap: 0, borderBottom: `1px solid ${C.bdr}`, marginBottom: 20 }}>
      {tabs.map((t, i) => (
        <button key={t.k} onClick={() => setTab(t.k)} style={{
          ...
          fontFamily: F.sans, fontSize: 15, fontWeight: tab === t.k ? 600 : 400,
...
function CompassCareer(){
  const lineR={display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${C.bdr}`,fontSize:17};
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:20}}>
      <div style={{background:C.bgS,borderRadius:4,padding:"12px 14px"}}>
        <div style={{fontSize:13,fontWeight:600,color:C.txT,textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:6}}>Now</div>
        <div style={{fontSize:16,fontWeight:600,color:C.tx,marginBottom:6}}>AI Data Ops Professional</div>
        {["Team lead","Annotator","QA","Transcriber","Translator"].map((r,i)=>(<div key={i} style={{fontSize:15,color:C.txS,padding:"2px 0"}}>{r}</div>))}
        <div style={{borderTop:`1px solid ${C.bdr}`,marginTop:6,paddingTop:6}}>
          {["Starter writer","Starter content creator"].map((r,i)=>(<div key={i} style={{fontSize:15,color:C.txS,padding:"2px 0"}}>{r}</div>))}
        </div>
        <div style={{borderTop:`1px solid ${C.bdr}`,marginTop:6,paddingTop:6}}>
          <div style={{fontSize:15,color:C.orange,fontWeight:500,padding:"2px 0"}}>Handshake AI - SPA i18n (in progress)</div>
        </div>
      </div>
      <div style={{background:C.orangeBg,borderRadius:4,padding:"12px 14px"}}>
        <div style={{fontSize:13,fontWeight:600,color:C.orange,textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:6}}>Building</div>
        {["Claude CCA learning","Claude CCA certification","Portfolio","Update CV + LinkedIn","Writing skills","Content creation skills","Trading / stock skills","Product design / UX","Personal brand / online presence","Networking / community"].map((r,i)=>(<div key={i} style={{fontSize:15,color:C.tx,padding:"2px 0"}}>{r}</div>))}
      </div>
      <div style={{background:C.greenBg,borderRadius:4,padding:"12px 14px"}}>
        <div style={{fontSize:13,fontWeight:600,color:C.green,textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:6}}>Goal</div>
        {["Implementation Specialist","Human Data Manager","AI Data Annotation Team Lead","Strategic Project Associate","Investor / Trader","Writer","Creator","Founder"].map((r,i)=>(<div key={i} style={{fontSize:15,color:C.tx,padding:"2px 0",fontWeight:i>=4?600:400}}>{r}</div>))}
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:0,marginBottom:6}}>
      {[{t:"Y1",p:"Specialist",ps:"$80-120K",f:"Side projects",fs:"First users",c:"First content",cs:"Writing + channel",v:"Learning",vs:"Paper trading"},{t:"Y2",p:"Senior Lead",ps:"$120-160K",f:"First revenue",fs:"$1-5K/mo",c:"Audience building",cs:"Consistent output",v:"Active trading",vs:"Small portfolio"},{t:"Y3-4",p:"Architect",ps:"$160-250K",f:"Product-market fit",fs:"$5-15K/mo",c:"Monetize",cs:"Paid content",v:"Growing portfolio",vs:"Passive income"},{t:"Y5+",p:"Director",ps:"$200-350K+",f:"Independence",fs:"Replaces salary",c:"Brand",cs:"Multiple formats",v:"Financial freedom",vs:"Compounding"}].map((y,i)=>(<div key={i} style={{padding:"14px 16px",borderBottom:`1px solid ${C.bdr}`,borderLeft:i>0?`1px solid ${C.bdr}`:"none"}}>
        <div style={{fontSize:15,fontWeight:600,color:C.txT,marginBottom:6}}>{y.t}</div>
        <div style={{fontSize:17,fontWeight:600,color:C.purple,marginBottom:2}}>{y.p}</div>
        <div style={{fontSize:16,color:C.txS,marginBottom:6}}>{y.ps}</div>
        <div style={{fontSize:17,fontWeight:600,color:C.gold,marginBottom:2}}>{y.f}</div>
        <div style={{fontSize:16,color:C.txS,marginBottom:6}}>{y.fs}</div>
        <div style={{fontSize:17,fontWeight:600,color:C.orange,marginBottom:2}}>{y.c}</div>
        <div style={{fontSize:16,color:C.txS,marginBottom:6}}>{y.cs}</div>
        <div style={{fontSize:17,fontWeight:600,color:C.blue,marginBottom:2}}>{y.v}</div>
        <div style={{fontSize:16,color:C.txS}}>{y.vs}</div>
      </div>))}
    </div>
    <div style={{display:"flex",gap:12,marginBottom:20}}>
      <span style={{fontSize:13,display:"flex",alignItems:"center",gap:4,color:C.txT}}><span style={{width:8,height:3,background:C.purple,borderRadius:1,display:"inline-block"}}/>Professional</span>
      <span style={{fontSize:13,display:"flex",alignItems:"center",gap:4,color:C.txT}}><span style={{width:8,height:3,background:C.gold,borderRadius:1,display:"inline-block"}}/>Founder</span>
      <span style={{fontSize:13,display:"flex",alignItems:"center",gap:4,color:C.txT}}><span style={{width:8,height:3,background:C.orange,borderRadius:1,display:"inline-block"}}/>Creative</span>
      <span style={{fontSize:13,display:"flex",alignItems:"center",gap:4,color:C.txT}}><span style={{width:8,height:3,background:C.blue,borderRadius:1,display:"inline-block"}}/>Investor</span>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <div style={{border:`1px solid ${C.bdr}`,borderRadius:4,padding:20}}>
        ...
        <div style={{fontFamily:F.sans,fontSize:22,fontWeight:600,color:C.tx,margin:"8px 0 4px"}}>AI Implementation Specialist</div>
      ...
      <div style={{border:`1px solid ${C.bdr}`,borderRadius:4,padding:20}}>
        ...
        <div style={{fontFamily:F.sans,fontSize:22,fontWeight:600,color:C.tx,margin:"8px 0 4px"}}>Independent Creator / Founder</div>
...
function LearningPlan(){const[exp,setExp]=useState("p1");
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:20}}>
      {[{l:"CCA Certification",v:doneActions+"/"+totalActions+" steps",c:doneActions===totalActions?C.green:C.orange},{l:"Product",v:"Evalynn OS v2 or Golden Era v2",c:C.txS},{l:"Portfolio",v:"App + README + Demo video",c:C.txS}].map((g,i)=>(<div key={i} style={{background:C.bgS,borderRadius:4,padding:"10px 12px"}}><div style={{fontSize:13,fontWeight:600,color:C.txT,textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:3}}>{g.l}</div><div style={{fontSize:16,fontWeight:500,color:g.c}}>{g.v}</div></div>))}
    </div>
    <div style={{display:"flex",justifyContent:"flex-end",marginBottom:16}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:14,color:C.txS}}>Start date</span>
        <input type="date" value={startDate||""} onChange={e=>setStartDate(e.target.value)} style={{border:`1px solid ${C.bdr}`,borderRadius:3,padding:"4px 8px",fontFamily:F.sans,fontSize:15,color:C.tx,background:"transparent",outline:"none"}}/>
      </div>
    </div>
    {GP_PHASES.map((ph,phIdx)=>{const isO=exp===ph.id;const clr=C[ph.color];const acts=getActions(ph);const skls=getSkills(ph);const dr=getDateRange(phIdx);const doneA=acts.filter(a=>a.done).length;const doneS=skls.filter(s=>s.done).length;return(<div key={ph.id} style={{marginBottom:4,border:`1px solid ${isO?clr+"40":C.bdr}`,borderRadius:4,background:isO?C.bg:C.bgS}}>
      <div onClick={()=>setExp(isO?null:ph.id)} style={{padding:"12px 16px",cursor:"pointer"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
          ...
          <span style={{fontFamily:F.mono,fontSize:15,color:C.txS}}>{ph.wk}{dr&&<span style={{marginLeft:6,fontSize:14,color:C.txT}}>{dr}</span>}</span>
          <span style={{marginLeft:"auto",fontSize:14,color:C.txT}}>{doneA}/{acts.length} steps, {doneS}/{skls.length} skills</span>
        </div>
      ...
      {isO&&<div onClick={e=>e.stopPropagation()} style={{padding:"0 16px 14px",borderTop:`1px solid ${C.bdr}`,paddingTop:12}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,alignItems:"start"}}>
          <div>
            <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em"}}>Actions</div>
            {acts.map(a=>(<div key={a.id} style={lineS}>
              ...
              <input value={a.notes||""} onChange={e=>updAction(ph.id,a.id,"notes",e.target.value)} style={{fontSize:14,color:C.txS,background:"transparent",border:"none",outline:"none",fontFamily:F.sans,width:80,textAlign:"right",flexShrink:0}} placeholder="notes"/>
            </div>))}
          </div>
          <div>
            <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em"}}>Skills to build</div>
            ...
            <div style={{marginTop:12}}>
              <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:4,textTransform:"uppercase",letterSpacing:"0.04em"}}>CCA domains</div>
              {ph.dom.map((d,i)=><div key={i} style={{fontSize:15,color:C.purple,padding:"2px 0"}}>{d}</div>)}
              <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:4,marginTop:8,textTransform:"uppercase",letterSpacing:"0.04em"}}>Milestone</div>
              <div style={{fontSize:15,color:clr}}>{ph.ms}</div>
            </div>
          </div>
        </div>
      </div>}
...
function IncomeTracker(){
  ...
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:20}}>
      <div style={{background:C.bgS,borderRadius:4,padding:14}}><div style={{fontFamily:F.sans,fontSize:14,color:C.txS,marginBottom:4}}>Monthly target</div><div style={{display:"flex",alignItems:"baseline",gap:4}}><span style={{fontSize:14,color:C.txS}}>$</span><input type="number" value={target} onChange={e=>setTarget(parseFloat(e.target.value)||0)} style={{fontSize:24,fontWeight:500,width:100,border:"none",padding:0,background:"transparent",outline:"none",fontFamily:F.sans,color:C.tx}}/></div><div style={{fontFamily:F.sans,fontSize:14,color:C.txT,marginTop:2}}>{(target*31).toLocaleString()} THB</div></div>
      <div style={{background:C.bgS,borderRadius:4,padding:14}}><div style={{fontFamily:F.sans,fontSize:14,color:C.txS,marginBottom:4}}>Estimated monthly</div><div style={{fontSize:24,fontWeight:500,color:totalEst>=target?C.green:C.orange}}>${Math.round(totalEst).toLocaleString()}</div><div style={{fontFamily:F.sans,fontSize:14,color:C.txT,marginTop:2}}>{Math.round(totalEst*31).toLocaleString()} THB</div></div>
      <div style={{background:C.bgS,borderRadius:4,padding:14}}><div style={{fontFamily:F.sans,fontSize:14,color:C.txS,marginBottom:4}}>Earned this month</div><div style={{fontSize:24,fontWeight:500,color:C.green}}>${Math.round(totalEarned).toLocaleString()}</div><div style={{fontFamily:F.sans,fontSize:14,color:C.txT,marginTop:2}}>{Math.round(totalEarned*31).toLocaleString()} THB</div></div>
    </div>
    <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em",display:"flex",justifyContent:"space-between"}}><span>Income streams</span><span onClick={addStream} style={{cursor:"pointer",fontSize:17,fontWeight:400}}>+</span></div>
    {streams.map((s,i)=>{const est=Math.round(s.rate*s.hrs*4);const sc=stabColors[s.stability]||stabColors.orange;return(
      <div key={i} style={lineS}>
        ...
        <span style={{fontSize:14,color:C.txT,flexShrink:0}}>$</span>
        <input type="number" value={s.rate} onChange={e=>updateStream(i,"rate",parseFloat(e.target.value)||0)} style={numIn} step="0.01"/>
        <span style={{fontSize:14,color:C.txT,flexShrink:0}}>/hr</span>
        <input type="number" value={s.hrs} onChange={e=>updateStream(i,"hrs",parseFloat(e.target.value)||0)} style={{...numIn,width:30}}/>
        <span style={{fontSize:14,color:C.txT,flexShrink:0}}>h/w</span>
        <span onClick={()=>cycleStability(i)} style={{fontSize:12,fontWeight:600,padding:"1px 6px",borderRadius:8,background:sc.bg,color:sc.tx,cursor:"pointer",flexShrink:0}}>{sc.label}</span>
        <input value={s.status} onChange={e=>updateStream(i,"status",e.target.value)} style={{...txtS,fontSize:14,color:C.txS,flex:1}} placeholder="Status"/>
        <span style={{fontSize:15,fontWeight:500,color:est>0?C.tx:C.txT,flexShrink:0,minWidth:50,textAlign:"right"}}>${est.toLocaleString()}</span>
        <span onClick={()=>{if(confirm("Remove "+s.name+"?"))removeStream(i);}} style={{cursor:"pointer",fontSize:14,color:C.txT,flexShrink:0}}>x</span>
      </div>);})}
    <div style={{marginTop:24}}>
      <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em",display:"flex",justifyContent:"space-between"}}><span>Next moves</span><span onClick={addMove} style={{cursor:"pointer",fontSize:17,fontWeight:400}}>+</span></div>
      {moves.map((m,i)=>{const ms=moveStatusCfg[m.status]||moveStatusCfg.wait;return(
        <div key={i} style={lineS}>
          <span onClick={()=>cycleMoveStatus(i,m.status)} style={{fontSize:12,fontWeight:600,padding:"1px 6px",borderRadius:8,background:ms.bg,color:ms.c,cursor:"pointer",flexShrink:0,userSelect:"none",minWidth:40,textAlign:"center"}}>{m.status||"wait"}</span>
          <input value={m.action} onChange={e=>updateMove(i,"action",e.target.value)} style={txtS} placeholder="What's the next move?"/>
          <span onClick={()=>removeMove(i)} style={{cursor:"pointer",fontSize:14,color:C.txT,flexShrink:0}}>x</span>
        </div>);})}
    </div>
    <div style={{marginTop:24}}>
      <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em",display:"flex",justifyContent:"space-between"}}><span>Work log</span><span onClick={addLog} style={{cursor:"pointer",fontSize:17,fontWeight:400}}>+</span></div>
      {logs.map((lg,i)=>{const tc={win:{bg:C.greenBg,c:C.green},decision:{bg:C.yellowBg,c:"#856d0a"},change:{bg:C.blueBg,c:C.blue},note:{bg:C.bgS,c:C.txS}};const lc=tc[lg.type]||tc.note;return(
        <div key={i} style={lineS}>
          <span style={{fontSize:14,color:C.txT,flexShrink:0,minWidth:42}}>{lg.date||""}</span>
          <span onClick={()=>cycleLogType(i,lg.type)} style={{fontSize:12,fontWeight:600,padding:"1px 6px",borderRadius:8,background:lc.bg,color:lc.c,cursor:"pointer",flexShrink:0,userSelect:"none",minWidth:48,textAlign:"center"}}>{lg.type||"note"}</span>
          <input value={lg.text} onChange={e=>updateLog(i,"text",e.target.value)} style={txtS} placeholder="What happened or changed?"/>
          <span onClick={()=>removeLog(i)} style={{cursor:"pointer",fontSize:14,color:C.txT,flexShrink:0}}>x</span>
        </div>);})}
    </div>
  </div>);
}
...
function SkillMatrix(){
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
      ...
      <div style={{fontSize:15,color:C.txS,marginBottom:12}}>Project Lead, i18n - Thai Language Operations</div>

      <div style={{background:C.greenBg,borderRadius:4,padding:"10px 14px",marginBottom:16}}>
        <div style={{fontSize:15,fontWeight:500,color:C.green}}>Goal: Make Thai the highest quality language pod. Data so good that clients specifically request us.</div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:10,marginBottom:20}}>
        {HS_PLAN.map(sec=>{
          ...
          return(<div key={sec.key} style={{background:C.bgS,borderRadius:4,padding:"10px 12px"}}>
            <div style={{fontSize:13,fontWeight:600,color:sec.color,textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:3}}>{sec.label.split(" - ")[0]}</div>
            <div style={{fontSize:22,fontWeight:500,color:pct>=80?C.green:pct>=40?"#856d0a":C.txT}}>{pct}%</div>
            <div style={{fontSize:14,color:C.txT}}>{done}/{allItems.length}</div>
          </div>);
        })}
      </div>
      ...
      {HS_PLAN.map(sec=>{
        ...
        return(<div key={sec.key} style={{marginBottom:20}}>
          <div style={{padding:"8px 12px",background:sec.bg,borderRadius:"4px 4px 0 0",borderBottom:`1px solid ${C.bdr}`}}>
            <div style={{fontSize:16,fontWeight:600,color:sec.color}}>{sec.label}</div>
            <div style={{fontSize:14,color:C.txS,marginTop:2}}>{sec.goal}</div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:sec.skills.length>0?"1fr 1fr":"1fr",gap:20,padding:"10px 0",alignItems:"start"}}>
            <div>
              <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em"}}>Actions ({doneA.length}/{sec.actions.length})</div>
              {todoA.map(a=>(
                <div key={a.key} style={lineS}>
                  ...
                  <span style={{fontSize:15,color:C.tx,flex:1}}>{a.text}</span>
                </div>
              ))}
              {doneA.length>0&&<div style={{background:C.bgS,borderRadius:4,padding:"6px 10px",marginTop:4}}>
                {doneA.map(a=>(
                  <div key={a.key} style={{padding:"4px 0",display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <span style={{fontSize:14,color:C.txT,textDecoration:"line-through",flex:1}}>{a.text}</span>
                  </div>
                ))}
              </div>}
            </div>
            {sec.skills.length>0&&<div>
              <div style={{fontSize:13,fontWeight:600,color:C.txT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em"}}>Skills ({doneS.length}/{sec.skills.length})</div>
              {todoS.map(s=>(
                <div key={s.key} style={lineS}>
                  ...
                  <span style={{fontSize:15,color:C.tx,flex:1}}>{s.text}</span>
                </div>
              ))}
              {doneS.length>0&&<div style={{background:C.bgS,borderRadius:4,padding:"6px 10px",marginTop:4}}>
                {doneS.map(s=>(
                  <div key={s.key} style={{padding:"4px 0",display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <span style={{fontSize:14,color:C.txT,textDecoration:"line-through",flex:1}}>{s.text}</span>
                  </div>
                ))}
              </div>}
            </div>}
...
function WhoIAm(){
  ...
  const Voice=({pl,q,color,bg})=><Cd accent={color} bg={bg}><div style={{fontFamily:F.mono,fontSize:14,color,fontWeight:700,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.06em"}}>{pl}</div><div style={{fontFamily:F.serif,fontSize:19,color:C.tx,fontStyle:"italic",lineHeight:1.7}}>"{q}"</div></Cd>;
  return(<div><H1 style={{fontSize:32}}>Who I Am</H1><Ps style={{marginBottom:20}}>Sun Pisces - Moon Virgo - Rising Virgo - March 18, 1992</Ps>
    <Collapse title="Natal Chart" icon="\u2605" color={C.gold} open={true}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:8,marginBottom:16}}>{[{l:"Sun",v:"Pisces 28\u00B0",c:C.pisces,bg:C.piscesBg},{l:"Moon",v:"Virgo 23\u00B0",c:C.virgo,bg:C.virgoBg},{l:"Rising",v:"Virgo 27\u00B0",c:C.virgo,bg:C.virgoBg},{l:"MC",v:"Gemini 27\u00B0",c:C.ocean,bg:C.oceanBg}].map(b=>(<div key={b.l} style={{background:b.bg,borderRadius:3,padding:"8px 14px"}}><div style={{fontFamily:F.mono,fontSize:13,color:b.c,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:3}}>{b.l}</div><div style={{fontFamily:F.sans,fontSize:17,fontWeight:600,color:C.tx}}>{b.v}</div></div>))}</div><div style={{border:`1px solid ${C.bdr}`,borderRadius:4,overflow:"hidden"}}><table style={{width:"100%",borderCollapse:"collapse",fontFamily:F.sans,fontSize:16}}><thead><tr style={{background:C.bgS}}>{["Planet","Sign","Deg","H","Notes"].map(h=><th key={h} style={{textAlign:"left",padding:"6px 10px",fontWeight:500,fontSize:14,color:C.txS,borderBottom:`1px solid ${C.bdr}`,textTransform:"uppercase",letterSpacing:"0.04em"}}>{h}</th>)}</tr></thead><tbody>{planets.map((p,i)=><tr key={p.p} style={{borderBottom:i<planets.length-1?`1px solid ${C.bdr}`:"none"}}><td style={{padding:"8px 10px",fontWeight:600,color:p.c}}>{p.p}</td><td style={{padding:"8px 10px"}}>{p.s}</td><td style={{padding:"8px 10px",fontFamily:F.mono,fontSize:15,color:C.txS}}>{p.d}</td><td style={{padding:"8px 10px",fontFamily:F.mono,fontSize:15,color:C.txT}}>{p.h}</td><td style={{padding:"8px 10px",color:C.txS}}>{p.n}</td></tr>)}</tbody></table></div></Collapse>
    ...
    <Collapse title="Seven Operating Rules" icon="\u2699" color={C.virgo}>{rules.map(r=>(<Cd key={r.n} accent={r.c} bg={r.bg}><div style={{display:"flex",gap:12}}><div style={{fontFamily:F.serif,fontSize:24,color:r.c,fontWeight:700,opacity:0.3,minWidth:24}}>{r.n}</div><div><P style={{fontWeight:600,marginBottom:3}}>{r.t}</P><Ps style={{marginBottom:0}}>{r.d}</Ps></div></div></Cd>))}</Collapse>
    ...
    <Collapse title="Crystal Kit" icon="\uD83D\uDC8E" color={C.gold}>{[{n:"Hessonite Garnet",p:"RAHU",d:"Calms hunger.",c:C.bloodRed,bg:C.bloodRedBg,t:"red"},{n:"Amethyst",p:"MOON-PLUTO",d:"Emotional transmutation.",c:C.pisces,bg:C.piscesBg,t:"purple"},{n:"Aquamarine",p:"VENUS",d:"Truth in love.",c:C.ocean,bg:C.oceanBg,t:"blue"},{n:"Carnelian",p:"MARS",d:"Creative fire.",c:C.aries,bg:C.ariesBg,t:"orange"},{n:"Labradorite",p:"ALL",d:"The unifier. Literally you.",c:C.gold,bg:C.goldBg,t:"yellow"}].map(s=>(<Cd key={s.n} accent={s.c} bg={s.bg}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><P style={{fontWeight:600,marginBottom:0}}>{s.n}</P><div style={{fontFamily:F.mono,fontSize:13,color:s.c}}>{s.p}</div></div><Tag color={s.t}>{s.n.split(" ")[0]}</Tag></div><Ps style={{marginTop:6,marginBottom:0}}>{s.d}</Ps></Cd>))}</Collapse>
  </div>);}
...
function LifeCompass(){
  ...
  const st={sec:{marginBottom:48},lbl:{fontFamily:F.mono,fontSize:14,letterSpacing:"0.12em",textTransform:"uppercase",color:C.txT,marginBottom:16},hd:{fontFamily:F.serif,fontSize:32,fontWeight:700,color:C.tx,lineHeight:1.3,marginBottom:20,letterSpacing:"-0.01em"},p:{fontFamily:F.sans,fontSize:18,color:C.tx,lineHeight:1.8,marginBottom:16},ps:{fontFamily:F.sans,fontSize:18,color:C.txS,lineHeight:1.8,marginBottom:16},sep:{height:1,background:C.bdr,margin:"48px 0"}};
  return(<div style={{}}>
    ...
    <p style={{fontFamily:F.serif,fontSize:22,color:C.tx,lineHeight:1.7,fontStyle:"italic",marginBottom:8}}>I will keep going. And I will keep refining what I want to do and what I can do in the future.</p>
    </div>
    <div style={st.sep}/>
    <div style={{padding:"48px 0 80px"}}><p style={{fontFamily:F.serif,fontSize:"clamp(22px,4vw,30px)",color:C.tx,fontWeight:700,fontStyle:"italic",lineHeight:1.4,marginBottom:24}}>My thing is the space where wellness, creativity, and technology overlap.</p><p style={{fontFamily:F.sans,fontSize:18,color:C.txS,lineHeight:1.7,marginBottom:28}}>This document is a living thing. It will change as I change. But the core won't move. The core is: I help people come back to themselves. Everything else is just the form it takes on any given Tuesday.</p><p style={{fontFamily:F.mono,fontSize:14,color:C.txT,letterSpacing:"0.1em"}}>Evalynn Jetipa</p><p style={{fontFamily:F.mono,fontSize:13,color:C.txT,marginTop:4}}>Bangkok, March 2026</p></div>
  </div>);}

function Year2026({month}){
  const mo=month||"mar";
  const TH=({children})=><th style={{textAlign:"left",padding:"8px 10px",fontWeight:500,fontSize:14,color:C.txS,borderBottom:`1px solid ${C.bdr}`}}>{children}</th>;
  const Row=({dy,hidden,onToggle,checked,onCheck,notes,onNotes,extraGame,onExtraGame,extraActual,onExtraActual})=>{const hasA=dy.actual&&dy.actual.length>0;const bg=dy.lv==="m"?C.goldBg:dy.lv==="s"?C.bgS:"transparent";
    ...
    const ta={width:"100%",marginTop:6,padding:"4px 6px",border:`1px solid ${C.bdr}`,borderRadius:3,fontFamily:F.sans,fontSize:14,color:C.tx,background:"transparent",resize:"vertical",outline:"none",boxSizing:"border-box",minHeight:28};
    return(<>
    <tr style={{borderBottom:hidden?`1px solid ${C.bdr}`:"none",background:hidden?C.bgS:bg,verticalAlign:"top",opacity:hidden?0.5:1,transition:"opacity 0.15s"}}>
      <td onClick={onToggle} style={{padding:hidden?"6px 10px":"8px 10px",borderRight:`1px solid ${C.bdr}`,width:110,minWidth:110,cursor:"pointer"}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <span style={{fontSize:13,color:C.txT,transition:"transform 0.15s",transform:hidden?"rotate(0)":"rotate(90deg)"}}>{"\u25B6"}</span>
          <div>
            <div style={{fontFamily:F.sans,fontSize:16,fontWeight:600,color:C.tx}}>{dy.d} <span style={{fontWeight:400,color:C.txT,fontSize:14}}>{dy.day||""}</span></div>
            {!hidden&&<div style={{fontFamily:F.sans,fontSize:14.5,color:C.txS,lineHeight:1.4,marginTop:2}}>{dy.t}</div>}
        ...
        {hidden&&<span style={{fontFamily:F.sans,fontSize:14,color:C.txT,marginLeft:16}}>{dy.t}</span>}
      </td>
      {hidden?<td colSpan={3} style={{padding:"6px 10px",fontSize:15,color:C.txT}}></td>:<>
        <td style={{padding:"8px 10px",borderRight:`1px solid ${C.bdr}`,fontSize:15,color:C.txS,lineHeight:1.65}}>{dy.pred}</td>
        <td style={{padding:"8px 10px",borderRight:`1px solid ${C.bdr}`,background:allGameItems.length>0?C.yellowBg:"transparent"}}>
          {uncheckedItems.length>0?uncheckedItems.map(r=>(
            <div key={r.idx} onClick={(e)=>{e.stopPropagation();onCheck(r.idx);}} style={{display:"flex",alignItems:"flex-start",gap:6,fontSize:15,color:C.tx,lineHeight:1.6,marginBottom:4,cursor:"pointer"}}>
          ...
          )):<span style={{fontSize:15,color:C.txT}}>{allGameItems.length>0?"All done":"\u2014"}</span>}
        ...
        <td style={{padding:"8px 10px",background:(hasA||checkedItems.length>0||extraActual||notes)?C.greenBg:""}}>
          {actualLines.map((l,j)=><div key={"a"+j} style={{fontSize:15,color:C.tx,lineHeight:1.6,marginBottom:1}}>{l}</div>)}
          {checkedItems.map((r)=>{const clean=r.text.replace(/[\u{1F300}-\u{1F9FF}\u{2600}-\u{27BF}\u{FE00}-\u{FEFF}\u{1FA00}-\u{1FAFF}]/gu,"").trim();return(
            <div key={"c"+r.idx} onClick={(e)=>{e.stopPropagation();onCheck(r.idx);}} style={{fontSize:15,color:C.tx,lineHeight:1.6,marginBottom:1,cursor:"pointer"}}>- {clean}</div>
          );})}
  ...
  const Tbl=({rows,intro,storageKey})=>{const[hiddenRows,setHiddenRows]=useState({});const[checkedMap,setCheckedMap]=useState({});const[notesMap,setNotesMap]=useState({});const[extraGameMap,setExtraGameMap]=useState({});const[extraActualMap,setExtraActualMap]=useState({});const[loaded,setLoaded]=useState(false);const[hideAll,setHideAll]=useState(false);
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
      ...
      <button onClick={toggleAll} style={{border:`1px solid ${C.bdr}`,background:C.bgS,borderRadius:3,padding:"4px 12px",fontFamily:F.sans,fontSize:15,color:C.txS,cursor:"pointer",whiteSpace:"nowrap",marginLeft:12}}>{hideAll?"Expand all":"Collapse all"}</button>
    </div>
    <div style={{border:`1px solid ${C.bdr}`,borderRadius:4,overflow:"hidden"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontFamily:F.sans,fontSize:16,tableLayout:"fixed"}}>
        ...
        <tbody>
          ...
          {hiddenList.length>0&&<tr><td colSpan={4} style={{padding:"6px 10px",background:C.bgS,fontSize:14,color:C.txT,borderBottom:`1px solid ${C.bdr}`}}>Hidden ({hiddenList.length})</td></tr>}
  ...
  return(<div>
    ...
    <P style={{fontFamily:F.serif,fontSize:22,lineHeight:1.7,marginBottom:20}}>The structures that couldn't hold your weight anymore - the roles that treated you as disposable, the patterns that kept you small, the identity built around serving others at the cost of yourself - they all came down. Not gently. Saturn in Pisces on your Sun made sure of that. 2026 is architecture. Not rebuilding what was. Building what should have been there all along.</P>
...
function UniversePage(){
  ...
  return(<div>
    <H1 style={{fontSize:32,margin:"0 0 4px"}}>My Universe</H1>
    <div style={{display:"flex",gap:0,borderBottom:`1px solid ${C.bdr}`,marginBottom:20}}>
      {tabs.map((t,i)=>(<button key={t.k} onClick={()=>setTab(t.k)} style={{padding:`8px 16px 8px ${i===0?0:16}px`,border:"none",background:"none",fontFamily:F.sans,fontSize:17,fontWeight:tab===t.k?600:400,color:tab===t.k?C.tx:C.txT,cursor:"pointer",borderBottom:tab===t.k?`2px solid ${C.tx}`:"2px solid transparent",marginBottom:-1}}>{t.l}</button>))}
    </div>
    {tab==="y2026"&&<div>
      <div style={{display:"flex",gap:0,marginBottom:16}}>{[{k:"mar",l:"March"},{k:"apr",l:"April"},{k:"may",l:"May"}].map((m,i)=>(<button key={m.k} onClick={()=>setSub2(m.k)} style={{padding:"5px 12px",border:"none",background:sub2===m.k?C.bgS:"transparent",borderRadius:4,fontFamily:F.sans,fontSize:16,fontWeight:sub2===m.k?600:400,color:sub2===m.k?C.tx:C.txT,cursor:"pointer"}}>{m.l}</button>))}</div>
      <Year2026 month={sub2}/>
    </div>}
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.tx, display: "flex" }}>
      ...
      <nav style={{
        ...
        <div style={{
          padding: "12px 20px 20px", fontFamily: F.serif, fontSize: 20, fontWeight: 700, color: C.tx,
        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            <button key={item.k} onClick={() => goTo(item.k)} style={{
              ...
              fontFamily: F.sans, fontSize: 15, fontWeight: isActive ? 600 : 400,
